...
const banners = [
  { id: '1', title: 'Traditional wears', text: 'Explore our collection of cultural wears', image: 'https://c8.alamy.com/comp/2A15MRK/exterior-of-clothes-shop-selling-traditional-indian-clothing-southall-west-london-england-uk-2A15MRK.jpg' },
...
const HomePageUser = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response1 = products; // Replace with API call
      setData(response1);
  ...
  const renderItem = ({ item }) => (
    // <Link href="" asChild>
      ...
      <View style={[styles.prodCenter, {justifyContent: 'space-between', paddingLeft:5, paddingRight:5 }]}>
        <Text style={styles.cardTitle}>{item.name}</Text>
        <Text style={styles.cardRating}>⭐ {item.rating}</Text>
      </View>
      <View style={[styles.prodCenter, {justifyContent: 'flex-start' }]}>
  ...
  return (
    <View style={styles.container}>
      ...
      <View>
        {/* <View style={{ flexDirection:'row',alignItems:'center', justifyContent:'space-between',padding:16, paddingTop:10, paddingLeft:0 }}>
          <Text style={{ fontWeight:'bold' }}>Categories</Text>
          <View style={{ flexDirection:'row', alignItems:'center',  justifyContent:'center' }}>
            <Text style={{ textAlign:'center', color:COLORS.warning }}>View All </Text>
            <FontAwesome name="angle-right" size={24} color={COLORS.warning} />
          </View>
        </View> */}
...
const styles = StyleSheet.create({
  ...
  bannerContent: { width: screenWidth, padding: 12, backgroundColor:COLORS.ColorHome, borderRadius: 9, height:150},
  bannerTitle: { fontSize: 18, fontWeight: 'bold', marginTop: 15},
  bannerText: { color: '#666', marginTop: 9},
  bannerButton: { backgroundColor: COLORS.ColorBrown, padding: 10, borderRadius: 5,  width:'25%', marginTop: 20 },
  bannerButtonText: { color: '#fff' },
  bannerImage: { width: '43%', height: 100, position: 'absolute', right: 10, top: 10 , borderRadius:10},
  dotContainer: { flexDirection: 'row', justifyContent: 'center', marginTop: 2 },
